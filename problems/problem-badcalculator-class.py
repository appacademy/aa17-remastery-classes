...
# 3. An instance method called `mult_nums` that returns the value of applying
#    the arithmetic operator `*` to the two instance properties.

# 4. An instance method called `div_nums ` that returns the value of applying
#    `/` to the two instances properties. It should divide the first number by
#    the second, and return a string explaining that it cannot perform this
#    operation if it is asked to divide by zero.

# 5. A `__repr__` method that returns the string: "Bad Calculator - can only do
